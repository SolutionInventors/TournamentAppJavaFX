...
public class ChallengeTest {
	...
	public static void main(String[] args) throws OnlyOutstandingAreLeftException, TournamentException {
		File file = new File("Arsenal.jpg");
		
		Competitor[] coms = { new Competitor("Chinedu", file), new Competitor("Chidi", file) };

		Tournament tournament = new Challenge( SportType.GOALS_ARE_NOT_SCORED, coms, 2);
	...
	public static void simulateRound(Tournament tournament) throws TournamentEndedException, OnlyOutstandingAreLeftException
	{
		...
		try {
			...
			Challenge challengeSpecific = (Challenge) tournament;
			Test.displayStandingTable(challengeSpecific.getStandingTable().getStringTable());
